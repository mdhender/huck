...
| T3.2 | DONE   |        | Removed double-submit CSRF plumbing end-to-end: `csrfToken` helper and all `CSRF` view fields, `_csrf` hidden form fields, and the `X-CSRF-Token` mirror in `app.js`. Tests no longer carry `_csrf` form values or `X-CSRF-Token` headers; the `jarHelper.value` peek method and the now-unused jar returns were dropped. DESIGN.md §8.5 rewritten to document the `http.CrossOriginProtection` + `SameSite=Lax` + HSTS layering; new §12.1 captures the Nginx TLS-1.3 deployment contract. Open question on production `ssl_protocols TLSv1.3;` recorded in §8.5 for sprint closure. |
