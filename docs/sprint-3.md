...
Status: **Closed 2026-05-10.** All tasks (T1–T15) landed on `main`; manual smoke test passed.
...
| Task | Status | Commit | Notes |
|------|--------|--------|-------|
| T1   | DONE   | 8e7d0cc | Folded the stdlib Mailgun HTTP client into `internal/mail`; deleted `internal/email/`; updated AGENTS.md, README.md, and docs/mailgun-lite.md. |
| T2   | DONE   | 9094832 | Repurposed `cmd/sendtest` as the documented Mailgun smoke-test: switched to `internal/config` + `ff.Parse` (`HUCK_MAILGUN_*` env vars unchanged), added a real `--to` flag, factored `Config.ValidateMailer`/`missingMailerFlags` shared with `ValidateServe`, and added `cmd/sendtest/README.md`. |
| T3.1 | DONE   | edb05ad | Replaced Echo's double-submit `_csrf` middleware with `http.NewCrossOriginProtection()` mounted as an Echo middleware adapter in `internal/server/middleware.go`; replaced `TestCSRFRejected` with `TestCrossOriginRejected` + `TestSameOriginAllowed`. Per the task scope, `csrfToken`, `CSRF` view fields, the `_csrf` form fields, and the `app.js` header mirror are intentionally left for T3.2; existing tests had `t.Fatal("no _csrf cookie set")` assertions that were dropped to keep the test suite green with the smallest diff. |
| T3.2 | DONE   | 9df4e7b | Removed double-submit CSRF plumbing end-to-end: `csrfToken` helper and all `CSRF` view fields, `_csrf` hidden form fields, and the `X-CSRF-Token` mirror in `app.js`. Tests no longer carry `_csrf` form values or `X-CSRF-Token` headers; the `jarHelper.value` peek method and the now-unused jar returns were dropped. DESIGN.md §8.5 rewritten to document the `http.CrossOriginProtection` + `SameSite=Lax` + HSTS layering; new §12.1 captures the Nginx TLS-1.3 deployment contract. Open question on production `ssl_protocols TLSv1.3;` recorded in §8.5 for sprint closure. |
| T4   | DONE   | b49d709 | Documented `runServe`'s migrate-but-never-create contract via doc comment in `cmd/huck/main.go`, expanded DESIGN.md §7.2/§7.3 with the same three-point contract, and added `TestMigrateAfterCreateIsNoOp` exercising the Create → Open → Migrate path the serve startup uses. AGENTS.md verification list was already correct. |
| T5   | DONE   | e8a956e | Honoured the `ctx` on `invites.Store.Consume`: rewrote the doc comment to describe both halves of the contract (fail-fast `ctx.Err()` + the conn's interrupt channel that `sqlitex.Pool.Take` already wires), replaced `_ = ctx` with the entry-check, and added an explanatory comment after `s.pool.Take` in `handleSignupSubmit`. New `TestConsumeCancelled` locks in the fail-fast behaviour and verifies the UPDATE never ran. |
| T6   | DONE   | 64271f7 | Added `fmtUTC(t) (display, iso)` in `internal/server/format.go` (kept as a Go helper rather than a template FuncMap — the four call sites all assemble per-row view structs, so a function fits the existing pattern). Replaced the hand-formatted literals in `rowViewAt`, `handleAdminUsersList`, and `newAdminUserView`; dropped the now-unused `time` import from `admin_users.go`. New `TestFmtUTC` locks the display/ISO format strings. |
| T7   | DONE   | 9e4502e | Added `hxRedirect(c, path)` next to `isHXFragmentRequest` in `render.go`; replaced the four "if HX-Request → 204+HX-Redirect, else 303" call sites in `handleLoginSubmit`, `handleLogout`, `handleSignupSubmit`, and `requireAuth`. New `TestHXRedirect` in `redirect_test.go` covers both branches table-driven. |
| T8   | DONE   | 1962439 | Extracted `passwordErrMsg(err) string` in `server.go` returning "" for non-password sentinels, so both `signupErrorMessage` and `adminUserPasswordErrorMessage` delegate then layer their own switch/fallback on top. Dropped the now-unused `fmt` import from `admin_users.go`. New `password_error_test.go` covers the three password sentinels, the empty-string fallthrough for nil/unrelated errors, and that both callers still produce their distinct generic-fallback messages. |
| T9   | DONE   | c4a07f2 | Replaced `invites.normaliseEmail` with a call to `users.Normalise`; the invites package now imports users directly (no cycle — users never needs invites, since the signup flow consumes an invite *and* creates a user from server, not the other way round). Rationale captured as a comment on the new import. |
| T10  | DONE   | 95f3533 | Replaced the `strings.Contains(err.Error(), "users.handle")` / `"invites.email"` driver-text classifiers with `sqlite.ErrCode == ResultConstraintUnique` plus a same-connection `SELECT 1` lookup against the normalised input. `users.classifyInsertErr` checks handle first then email; `invites.classifyInsertErr` confirms the active-invite partial index. Dropped the now-unused `strings` import from `invites.go`. Extended `TestUniqueConstraints` with a both-columns-conflict case that pins the handle-first ordering; the invite path is already covered by the existing case-insensitive `TestCreateDuplicateActive`. |
| T11  | DONE   | b2b107d | Replaced the `/admin` redirect with a minimal dashboard page (`pages/admin.html` + `adminIndexView`); `/admin/` is canonicalised to `/admin` via a single 301 registered outside the admin group so the redirect runs before `requireAdmin`. Added `/account` mounted under `requireAuth()` — handler reuses `newAdminUserView` so the data shape matches `/admin/users/:id` per the sprint task, with a new `pages/account.html` that drops the admin-only edit/delete actions. Home `home_authed.html` now links to `/account` for everyone and `/admin` for admins. New `TestAdminIndexAdminRendersDashboard`, `TestAdminIndexTrailingSlashRedirects`, and `account_test.go` (anonymous-redirect, user-renders, admin-link visibility, deleted-user-clears-cookie). DESIGN.md §10 routes table refreshed with the new and previously-undocumented `/admin/users/:id*` rows. |
| T12  | DONE   | 6f94197 | Lowercased `BcryptCost`→`bcryptCost`, `MinHandleLen`→`minHandleLen`, `MaxHandleLen`→`maxHandleLen` — all three were consumed only inside `internal/auth`. Switched `validate_test.go` from `package auth_test` to `package auth` (internal test) so it can keep referencing the boundary constants without re-exporting them; `auth_test.go` stays `package auth_test`. `DefaultTokenTTL` was listed in the task but is actually consumed by `internal/server/server.go` (Issue calls + cookie MaxAge), so it remains exported. |
| T13  | DONE   | 9febb74 | Confirmed `templates/email/invite.html` uses `{{ .URL }}` only inside the `<a href="…">` attribute (HTML-safe context, no JS/CSS). Documented the flat-`Config`-struct rationale + ~25-field split threshold in `internal/config/config.go`. Refreshed the stale "(parsed but unused in Sprint 1)" comment on `BaseURL` — it has been required + used by `ValidateServe`/`admin_invites.go` since Sprint 2. The repo-wide grep for "Sprint 1 unused" / "Sprint 2 unused" turned up no other obsolete comments; the remaining Sprint references are sourcing/rationale citations. |
| T14  | DONE   | e19c3f0 | Split `homeView` into `homePublicView` (empty struct — Sprint 4 will hang auth-shell data off it) and `homeAuthedView` (Handle, Admin) and updated `handleHome` to construct each directly. The Sprint 4 doc route examples were verified to already use `/admin/users/:id` and `/admin/users/:id/edit` — no `:handle` references remained anywhere in `docs/`, so no edit was needed. Added `TestRendererDispatch` in `internal/server/render_test.go` covering all three renderer branches (page + no HX → layout; page + HX-Request → content only; partial → alone), pinning the baseline before Sprint 4 T2.2 changes layout dispatch. |
| T15  | DONE   | d04b35d | Doc-only readiness sweep. Verified the app-shell data contract is consistent across `docs/front-end-plan.md` §5 and `docs/sprint-4.md` T1.1/T2.1: typed shell view carries `Crumbs []Crumb`, renderer may compose shell+page once while pages keep their own page view as dot. Confirmed both docs treat `/admin` as a real dashboard and `/account` as a real signed-in page (front-end-plan §3, sprint-4 entry checklist + T1.3). Fixed one stale typo: sprint-4 T2.1 listed `.Page.CSRF` as an example wrapped field — CSRF view fields were removed in T3.2 — replaced with `.Page.Handle` (a real field on `homeAuthedView`). No other stale Sprint-3-baseline references found in the Sprint 4 docs. Standard `go build`/`vet`/`test` ran clean despite the doc-only diff. |
...
- **2026-05-10** — Closed. All 15 task rows landed on `main` with
  per-task commits backfilled into the table; closing verification
  (`go build`/`vet`/`test`, fresh-DB create + idempotent migrate,
  `cmd/sendtest` Mailgun round-trip, and the `/admin`, `/admin/`,
  `/account` route smoke tests) passed.
