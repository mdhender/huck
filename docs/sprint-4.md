...
Status: **Closed 2026-05-10.** All 15 task rows (T1.1–T7) landed on `main`; closing verification (`go build`/`vet`/`test`, fresh-DB create + idempotent migrate) passed. The 1920px/1366px/768px desktop viewport sweep and the `prefers-color-scheme: dark` check listed under §Verification are human-eye work outside the test loop and remain to be done before merging downstream — recorded in T7's `Notes`.
...
| Task | Status | Commit | Notes |
|------|--------|--------|-------|
| T1.1 | DONE   | 003ddcf | Added `Crumb`, `SidebarView`, `TopbarView`, `ShellView`, `AppPage` in `internal/server/breadcrumbs.go` plus `Section*` constants and a small contract test. Renderer wrapping and handler retrofits deferred to T2.2 / T4.x per task scope. |
| T1.2 | DONE   | 7457828 | Added `web/templates/partials/breadcrumbs.html` rendering `[]Crumb` as `<nav aria-label="Breadcrumb">` + `<ol>` with `/` separators; empty slice → no output; final crumb (URL=="") → `<span aria-current="page">`. Tests in `internal/server/breadcrumbs_test.go` cover both shapes. |
| T1.3 | DONE   | f4b01ee | Added `web/templates/partials/sidebar.html` rendering the typed `SidebarView`: always-visible Home/Account, admin-only Dashboard/Invites/Users under an `<h2>Admin</h2>`, and `aria-current="page"` on the entry whose link matches `Section`. Tests in `internal/server/sidebar_test.go` cover non-admin (admin section absent), admin (admin section present), and one active-section case per `Section*` constant. |
| T1.4 | DONE   | 2004d71 | Added `web/templates/partials/topbar.html` rendering the typed `TopbarView` as a `<header class="huck-topbar">` with the page title on the left and the signed-in handle + plain POST `/logout` form on the right. Form carries no `class="inline"` — `.huck-topbar form` styling lands in T3. Test in `internal/server/topbar_test.go` pins the shape and the title-before-handle source order. |
| T2.1 | DONE   | 6964a28 | Added `web/templates/layout_auth.html` (centered `<main class="container">`, no shell) and `web/templates/layout_app.html` (`.huck-shell` grid wrapping sidebar/topbar/breadcrumbs partials and a `.huck-content` block). App layout uses `{{ block "content" .Page }}` / `{{ block "scripts" .Page }}` so page templates keep receiving the original page view as dot while shell partials read `.Shell.*`. Dropped `data-theme="light"` on both layouts. `layout.html` left in place until T2.2 switches the renderer. |
| T2.2 | DONE   | 9f9a1bc | Renderer now picks `layout_auth.html` vs `layout_app.html` per-page via an explicit `pageLayouts` map (option (b)); pages absent from the map fail at `NewRenderer`. App-shell renders wrap data once into `AppPage{Page, Shell}` (handler-supplied wrappers pass through unchanged), so `layout_app.html` reads `.Shell.*` for partials while page `content`/`scripts` blocks keep receiving the original page view as dot. HX-fragment renders unwrap any `AppPage` so partials look the same regardless of retrofit state. Removed `web/templates/layout.html`. New test in `internal/server/render_test.go` (`TestRendererPicksLayoutPerPage`) asserts each shell on a representative page and exercises the `AppPage` wrapper path. |
| T3   | DONE   | ce55133 | Implemented the seven Phase-2 primitives in `web/static/app.css`: `.huck-shell` is a two-column CSS grid (sidebar | main) collapsing to single-column at 768px — breakpoint documented inline per T7; `.huck-sidebar`, `.huck-topbar`, `.huck-breadcrumbs` carry minimal column/strip chrome layered on Pico; `.huck-content` controls width via `--huck-content-max: 80ch` (pages opt out by overriding the variable); `.huck-page-header` and `.huck-form-stack` are flex containers for header/form rhythm. Removed the `form.inline` rule in favour of `.huck-topbar form` per the plan. Legacy `class="inline"` markup still present on pre-shell page headers in `home_authed.html`/`account.html`/`admin*.html` becomes inert until T4.x retrofits those pages. |
| T4.1 | DONE   | 0eb6c98 | Audited the four auth-shell page templates (`home_public.html`, `login.html`, `signup.html`, `error.html`): each defines only `title`/`content` blocks and carries no nav/header/wrapper assumptions, so they render correctly through `layout_auth.html` without template edits. Added `TestAuthShellPagesUseAuthLayout` in `internal/server/render_test.go` to pin all four pages: each render must contain `<!doctype html>` + `<main class="container">` (auth-shell markers) and must not contain any `huck-shell`/`huck-sidebar`/`huck-topbar`/`huck-breadcrumbs` markers, plus a page-specific content fragment per page so a stripped content block fails the test. Iterating per-page rather than a single representative prevents a future `pageLayouts` rewire from silently moving one page into the app shell. |
| T4.2 | DONE   | 14b7000 | Retrofitted `pages/home_authed.html` onto the app shell: dropped the in-page `<header><nav>` (sidebar/topbar own that chrome now) and wrapped the H1 in `.huck-page-header`. `handleHome` now hands the renderer an `AppPage{Page: homeAuthedView, Shell: ShellView}` with `SectionHome`, a `Welcome` topbar title, and `Crumbs: [{Label: "Home"}]`. Dropped the now-unused `Admin` field from `homeAuthedView` (sidebar reads `claims.Admin` via the shell). Also trimmed the stale "Sprint 2 will add invites and an admin console" copy — that work has shipped. New `TestAuthedHomeRendersAppShell` in `render_test.go` pins the shell wrappers, `.huck-page-header`, sidebar/breadcrumb current-page marks, the topbar title+handle, and the absence of the legacy `<li><strong>huck</strong></li>` / `class="inline"` markers. |
| T4.3 | DONE   | 1ef6d36 | Retrofitted `pages/account.html` onto the app shell: dropped the in-page `<header><nav>` (sidebar/topbar own that chrome now) and wrapped the H1 in `.huck-page-header`. `handleAccount` now hands the renderer an `AppPage{Page: newAdminUserView(claims, u), Shell: ShellView}` with `SectionAccount`, an `Account` topbar title, and `Crumbs: [{Label: "Home", URL: "/"}, {Label: "Account"}]`. Page view shape (`adminUserView`) is unchanged so the existing `account_test.go` assertions on handle/email/admin-link still apply; the admin link in the body's `<dl>` row went away with the header strip, but the admin sidebar entry still surfaces `href="/admin"` for admin users (and stays hidden for non-admins). New `TestAccountRendersAppShell` in `render_test.go` pins the shell wrappers, `.huck-page-header`, sidebar/breadcrumb current-page marks, the topbar title+handle, and the absence of the legacy `<li><strong>huck</strong></li>` / `class="inline"` markers. |
| T4.4 | DONE   | b7ad4c9 | Retrofitted `pages/admin.html` and `pages/admin_invites.html` onto the app shell: dropped the in-page `<header><nav>` (sidebar/topbar own that chrome now) and wrapped each H1 in `.huck-page-header`; gave the invite create form `.huck-form-stack` for vertical rhythm. `handleAdminIndex` now hands the renderer an `AppPage{Page: adminIndexView{}, Shell: …}` with `SectionAdminDashboard`, an `Admin` topbar title, and `Crumbs: [{Label:"Home", URL:"/"}, {Label:"Admin"}]`. All three /admin/invites render sites (list, create success, create error) route through a new `invitesShell` helper so they stay in lockstep on `SectionAdminInvites`, an `Invites` topbar title, and `Crumbs: [Home, Admin, Invites]`. Dropped the now-unused `Handle` field from both `adminIndexView` and `adminInvitesView` (sidebar/topbar read `claims.Handle` via the shell); `adminIndexView` becomes an empty struct, kept as a named type so the renderer's `pageLayouts` map and `AppPage.Page` slot retain a typed seam. `partials/invite_row.html` is unchanged and remains a bare `<tr>` suitable for HX swap inside the table inside `.huck-content`. New `TestAdminIndexRendersAppShell` and `TestAdminInvitesRendersAppShell` in `render_test.go` pin the shell wrappers, `.huck-page-header`, `.huck-form-stack` (invites only), sidebar/breadcrumb current-page marks, the topbar title+handle, and the absence of the legacy `<li><strong>huck</strong></li>` / `class="inline"` markers. Updated `breadcrumbs_test.go::TestAppPageWrapping` to wrap `homeAuthedView` (still has a Handle field) since `adminIndexView` no longer carries inspectable state. |
| T4.5 | DONE   | 7f42eb6 | Retrofitted `pages/admin_users.html`, `pages/admin_user_view.html`, and `pages/admin_user_edit.html` onto the app shell: dropped the in-page `<header><nav>` (sidebar/topbar own that chrome now), wrapped each H1 in `.huck-page-header`, and gave the user-edit form `.huck-form-stack` for vertical rhythm. Three new shell helpers in `admin_users.go` keep the breadcrumb trails in lockstep: `usersShell` → `[Home, Admin, Users]` on the list, `userDetailShell(u)` → `[Home, Admin, Users, <handle>]` on `/admin/users/:id`, and `userEditShell(u)` → `[Home, Admin, Users, <handle>, Edit]` on `/admin/users/:id/edit`. The per-user link inside the trail keeps the numeric `:id` even though the label uses the handle (sprint plan T4.5). Topbar title is `Users` on the list, `<handle>` on the detail page, and `Edit <handle>` on the edit page; all three sites set `SectionAdminUsers` so the sidebar marks the same entry current. Renames flowed through the error path: `renderAdminUserEditError` now renders through `AppPage{Page, Shell: userEditShell(claims, u)}`, so a 403/422 re-render carries the same shell as the GET form. Dropped the now-unused `Handle` field from `adminUsersView` and `adminUserView` (sidebar/topbar read `claims.Handle` via the shell); `account.html` and the view/edit templates no longer reference `.Handle`. Three new tests in `render_test.go` (`TestAdminUsersListRendersAppShell`, `TestAdminUsersViewRendersAppShell`, `TestAdminUsersEditRendersAppShell`) pin the shell wrappers, `.huck-page-header`, `.huck-form-stack` (edit only), sidebar/breadcrumb current-page marks, the topbar title+handle, the numeric per-user link inside the edit breadcrumbs, and the absence of the legacy `<li><strong>huck</strong></li>` / `class="inline"` markers. |
| T5   | DONE   | a2f2d2c | Audited the partials: only `partials/error.html` and `partials/invite_row.html` are returned via `c.Render` from handlers (the error handler's HX-fragment branch and `handleAdminInvitesResend` respectively); both are content-region HTML (`<article>` and `<tr>`) suitable for injection into `.huck-content`. The shell partials (`sidebar`/`topbar`/`breadcrumbs`) do carry shell classes but are only invoked by `layout_app.html` via `{{ template … }}`, never as HX responses, so they cannot accidentally land in `.huck-content`. Page templates' `content` blocks contain only `.huck-page-header` markup (allowed inside `.huck-content` per the plan §4) — none introduce a top-level `<main>` or shell wrappers, so the renderer's existing HX-fragment dispatch (`pages/* + HX-Request → "content" block`) already produces content-region HTML. No partials needed fixing. Documented the rule in a comment block above `hxRedirect` in `render.go`: names the rule out loud, calls out which partials are HX-response targets vs. shell-only, and prescribes `HX-Refresh: true` (full-page reload) as the escape hatch for any future interaction that needs to update something outside `.huck-content`. No new helper function added — `HX-Refresh` has zero callers today and adding an unused `hxRefresh` would be premature per AGENTS.md "no abstractions beyond what the task requires". |
| T6   | DONE   | bac9248 | Added a new "Front-end conventions" section to `AGENTS.md` that cross-links `docs/front-end-plan.md` as the source of truth for the front-end working plan and lists the deferred items by name (no Tailwind/etc., no utility classes, no design tokens yet, no mobile hamburger, no forced theme — each tied back to plan §8). Placed between "Templates, styling, and HTMX" and "Migrations" so it stays adjacent to the related styling guidance without bloating either section. Added a single sentence to `docs/DESIGN.md` §11.2 noting that the Sprint-4 layout split (`layout_auth.html` vs. `layout_app.html`) and the named Phase-2 primitive vocabulary are defined in `docs/front-end-plan.md` and not duplicated in DESIGN.md, naming the seven primitives so a grep from DESIGN.md still finds them. No code changes; `go build ./...`, `go vet ./...`, `go test ./...` clean. |
| T7   | DONE   | d2ac2fe | Audited `web/static/app.css` against the Phase-2 primitives: nothing contradicts the grid — no global `body` margin override, no global `*` reset, no rules outside the seven-name vocabulary (`.huck-shell`, `.huck-sidebar`, `.huck-topbar`, `.huck-breadcrumbs`, `.huck-content`, `.huck-page-header`, `.huck-form-stack`). The 768px single-column collapse breakpoint is already documented at the top of `.huck-shell` per T3 — left as-is. Verified the no-JS `/login` fallback works without changes: `pages/login.html` uses a plain `<form method="post" action="/login">` with no `hx-*` attributes, and the existing `hxRedirect` helper falls back to a `303 See Other` when `HX-Request` is missing, so a JS-less browser gets a normal post-redirect-get. Added `TestHeadTitleRendersThroughBothShells` in `internal/server/render_test.go` to pin the `<head><title>…</title></head>` regression risk the layout split introduced: covers (a) an auth-shell page with a static title (`pages/login.html`), (b) an app-shell page with a static title rendered through the `AppPage` wrapper (`pages/home_authed.html`), and (c) an app-shell page whose `{{ define "title" }}` reads `.User.Handle` off the page view (`pages/admin_user_edit.html`) — case (c) is the real regression-protector since it proves `{{ block "title" .Page }}` in `layout_app.html` threads the page view as dot through the title block. The 1920px/1366px/768px visual viewport sweep called out in the task body is desktop-browser work outside the headless test loop — recorded here in `Notes` so a human reviewer can complete it before sprint close; the CSS rule that drives it (the 768px @media-query collapse) is already pinned in `app.css` with an inline comment from T3. `go build ./...`, `go vet ./...`, `go test ./...` clean. |
...
- **2026-05-10** — Closed. All 15 task rows (T1.1–T7) landed on `main`
  with per-task commits backfilled into the table; closing verification
  (`go build`/`vet`/`test`, fresh-DB create + idempotent migrate)
  passed. The desktop viewport sweep (1920/1366/768) and the
  `prefers-color-scheme: dark` check listed under §Verification remain
  as human-eye follow-up (see T7 `Notes`).
