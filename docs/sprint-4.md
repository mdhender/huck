...
Status: **Closed 2026-05-10.** All 15 task rows (T1.1–T7) landed on `main`; closing verification — `go build`/`vet`/`test`, fresh-DB create + idempotent migrate, the desktop viewport sweep (1920/1366/768), and the `prefers-color-scheme: dark` check from §Verification — all passed.
...
- **2026-05-10** — Closed. All 15 task rows (T1.1–T7) landed on `main`
  ...
  (`go build`/`vet`/`test`, fresh-DB create + idempotent migrate, the
  1920/1366/768 desktop viewport sweep, and the `prefers-color-scheme:
  dark` check from §Verification) all passed.
